...
class class_qa_settings_page  {
	...
	public function qa_settings_options($options = array()){
		...
		$section_options = array(
			...
			'music_press_member_page_id'=>array(
				'css_class'=>'music_press_member_page_id',
				'title'=>__('User profile page', 'music-press-member'),
				'option_details'=>__('Select user profile page where the shortcode placed <pre>[user_profile]</pre>', 'music-press-member'),
				'input_type'=>'select', 
				'input_args'=> $class_music_press_member_functions->get_pages_list(),
			),
			'music_press_member_edit_page_id'=>array(
				'css_class'=>'music_press_member_edit_page_id',
				'title'=>__('User profile edit page', 'music-press-member'),
				'option_details'=>__('Select user profile edit page where the shortcode placed <pre>[music_press_member_edit]</pre>', 'music-press-member'),
				'input_type'=>'select', 
				'input_args'=> $class_music_press_member_functions->get_pages_list(),
			),
		);
		$options['<i class="fa fa-folder"></i> '.__('Pages', 'music-press-member')] = apply_filters( 'qa_settings_section_question_post', $section_options );
				...
				elseif( $option_info['input_type'] == 'file' ){
					...
					jQuery("#upload_button_'.$option_key.'").click(function(e) {
						...
						custom_uploader = wp.media.frames.file_frame = wp.media({
							title: "Choose File",
							button: { text: "'.__('Choose File', 'music-press-member').'" },
							multiple: false
						});
...
if(empty($_POST['qa_hidden']))
	...
	{	
		if($_POST['qa_hidden'] == 'Y') {
			...
			?>
			<div class="updated"><p><strong><?php _e('Changes Saved.', 'music-press-member' ); ?></strong></p></div>
...
<div class="wrap">
	...
	<div id="icon-tools" class="icon32"><br></div><?php echo "<h2>".sprintf(__('%s Settings', 'music-press-member'), MUSIC_PRESS_MEMBER_PLUGIN_NAME)."</h2>";?>
...
<p class="submit">
                    <input class="button button-primary" type="submit" name="submit" value="<?php _e('Save Changes','music-press-member' ); ?>" />
                </p>
		</form>
